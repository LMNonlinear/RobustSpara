...
function [mu, h, baseline, spectrum_detrended] = find_largest_k_peaks(frequency, spectrum, k, f_min, f_max, min_distance)
    frequency = frequency(:);
    spectrum = spectrum(:);
    ...
    try
        % Fit a smoothing spline with only two knots (start and end)
        % This is effectively a linear fit, but using spline for consistency
        % To enforce two knots, duplicate the first and last points
        % freq_extended = [frequency(1); frequency; frequency(end)];
        % spectrum_extended = [spectrum(1); spectrum; spectrum(end)];
        % 
        % spline_fit = spline(freq_extended, spectrum_extended, frequency);
        % baseline = spline_fit;
        h_smooth=0.5;
