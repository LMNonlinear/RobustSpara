...
if any(violatedFixedBnds_idx)
    % disp('Warning: x0 violates fixed bounds. Resetting to fixed bound.')
    x0(violatedFixedBnds_idx)=lb(violatedFixedBnds_idx);
end
if any(violatedLowerBnds_idx) 
    % disp('Warning: x0 violates lower bounds. Resetting to lower bound.')
...
if any(violatedUpperBnds_idx) 
    % disp('Warning: x0 violates upper bounds. Resetting to upper bound.')
