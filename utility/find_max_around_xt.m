...
for i=nxt:-1:1
    ...
    idx = cell(ndimsy, 1);
    for iDim = ndimsy:-1:1
        % if xt_min(i,iDim)>xi{iDim}(end)
        %     warning([num2str(i),' target ',num2str(iDim), ' dim reach the upper bound'])
        % end

        % if xt_max(i,iDim)<xi{iDim}(1)
        %     warning([num2str(i),' target ',num2str(iDim), ' dim reach the lower bound'])
        % end
    ...
    % % Concatenate the grid indices into a single matrix
    % all_indices = cat(ndimsy+1, grid_indices{:});

    % % Reshape the array and index matrices into 2D matrices
    % y_2d = reshape(y, [], size(y, ndimsy));
    % x_2d = reshape(x, [], size(y, ndimsy));
    % all_indices_2d = reshape(all_indices, [], ndimsy);
    ...
    % y_vec=reshape(y,[],ndimsy);
    % y_vec=
    % y_loc=y_in_range(index_vectors);
