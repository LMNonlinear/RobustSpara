...
methods
    ...
    function set.hos(self, val)
        ...
        if ~isempty(val)
            ...
            self.hos.svar = [];
            % self.svar = mean(self.svar, 2, 'omitnan');
            self.shatvar = [];
            ...
            % use the mean of bsvar
            % self.bsvar = mean(self.bsvar, 3, 'omitnan');

            % self.hos.bsdenom = tril2full(self.hos.bsdenom, 0, false);
            % self.hos.bsdenom = sqrt(mean(self.hos.bsdenom .^ 2, 3, 'omitnan'));
            % if self.hos.log_var_s
            %     self.hos.bsdenom = sqrt(exp(mean(log(self.hos.bsdenom .^ 2), 3, 'omitnan')));
            % else
            %     self.hos.bsdenom = sqrt(mean(self.hos.bsdenom .^ 2, 3, 'omitnan'));
            % end
    ...
    function get_bifrequency(self)
        ...
        % self.para_bs.idx_fxfy=logical(mat2tril(true(size(self.fx)),0,false));
    ...
    % function fx = get.fx(self)
    %     [fx, ~] = ndgrid(self.f);
    % end

    % function fy = get.fy(self)
    %     [~, fy] = ndgrid(self.f);
    % end
    ...
    % function comp = get.comp(self)
    %     comp = pred_comp(self.f, self.ks, self.kh, self.para_xi, self.para_alpha, self.para_bs, self.hos_components);
    % end
        ...
        if ~isempty(self.bs)
            ...
            switch self.normalization
                ...
                otherwise
                    ...
                    % bsdenom = sqrt(mean(self.sxsysxy, 3, 'omitnan'));
            ...
            switch self.normalization
                ...
                otherwise
                    ...
                    if isfield(self.para_bs, 'bshatdenom_pre') && ~isempty(self.para_bs.bshatdenom_pre)
                        ...
                        % bshatdenom = sqrt(mean(self.sxsysxyhat, 3, 'omitnan'));
                    end
    ...
    function bshatvar = get.bshatvar(self)
        ...
        if ~isempty(self.bs)
            ...
            % warning('have not check, should not use it in calculation')
    ...
    function [t, ci, se] = calc_bc_t(self, component, ispolar)
        ...
        % if isempty(self.jac_bc2x)
        %     self.jac_bc2x.(comp) = calc_jac_bc2x(self, comp, ispolar); %#ok<*PROPLC>
        %     jac_bc2x = self.jac_bc2x.(comp);
        % else
        %     jac_bc2x = self.jac_bc2x.(comp);
        % end
        ...
        % [ci, se] = nlpar_ci_se(bchat, self.model.residual, 'jacobian', jac);
    ...
    function [bchat] = x2bc(self, component, para, ispolar)
        ...
        % para_xi.kernel.para.h = para_xi_perturb.kernel.para.h;

        para_alpha = self.para_alpha;
        % para_alpha.kernel.para.h = para_alpha_perturb.kernel.para.h;
    ...
    % function bc_jac = get.bc_jac(self, comp)
    %     % jacobian of bc to h
    %     fun_h2bc = @(h) self.h2bc(comp, h);
    %     [bc_jac, err] = jacobianest(fun_h2bc, self.para_bs.kernel.h(:));
    % end

    % function bchat = h2bc(self, comp, h)
    %     para_bs = self.para_bs;
    %     para_bs.kernel.h = reshape(h, size(para_bs.kernel.h));
    %     % bshat = pred_bs(self.f, para_bs, self.fxfy);
    %     bchat = self.bchat_component(comp, para_bs);
    %     bchat = bchat(:);
    % end
    ...
    function r2w = get.r2w_s(self)
        % [r2s] = r2_s(self.s, self.shat, self.svar);
        % [r2w] = calc_r2_s(self.s, self.shat, self.svar);
        y = [self.s];
        yhat = [self.shat];
        yvar = [self.svar];
        r2w = calc_r2w(y, yhat, yvar);
    ...
    function r2 = get.r2_bs(self)
        % [r2bs] = r2_bs(self.bc, self.bshat, self.bshatdenom, self.dn_reg)
        Ns = self.Ns;
        bs = reshape(self.bs, [], Ns);
        y = [real(bs); imag(bs)];
        bshat = reshape(self.bshat, [], 1);
    ...
    function r2w = get.r2w_bs(self)
        % [r2w] = calc_r2_bs(self.bc, self.bshat, self.bshatdenom, self.dn_reg);
        Ns = self.Ns;
        bs = reshape(self.bs, [], Ns);
        y = [real(bs); imag(bs)];
        bshat = reshape(self.bshat, [], 1);
    ...
    function nllh = get.nllh(self)
        ...
        % nllh_s = sum(abs(calc_loss_s(self, self.loss_type, self.s, self.shat, self.svar).^2)); %sep_complex
        % nllh_s=mean((self.shat - self.s) .^ 2);
        % nllh_bs = mean(abs(calc_loss_bs(self, self.loss_type, bc, self.bshat, [], bshatvar).^2)); %sep_complex
        % nllh=nllh_s+nllh_bs;
        if imag(nllh) ~= 0
            nllh = nan;
        end

        % plot([abs(self.s),abs(self.shat),abs(self.s)-abs(self.shat)]);
        % plot([abs(self.bs(self.idxused)),abs(self.bshat(self.idxused)),abs(self.bs(self.idxused))-abs(self.bshat(self.idxused))]);
    end
    ...
    function dht = get.dht(self)
        % dht=10*log10(sum(self.para_alpha.kernel.para.h(2:end))/(self.para_alpha.kernel.para.h(1)));%db scale
    ...
    function summary = get.summary(self)
        ...
        % summary.dof = self.dof;
        % summary.likelihood = self.lh;
        % summary.loglikelihood = self.llh;
