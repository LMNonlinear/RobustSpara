...
if ~isempty(self.lambda) && ((isscalar(self.lambda) && self.lambda ~= 0) || length(self.lambda) > 1)
    ...
    switch self.regularization
        ...
        case {'L2', 'l2'}
            ...
            %     penalty=self.lambda(1)*norm([para_xi.kernel.para.h(:);para_xi.kernel.para.h(:);para_alpha.kernel.para.h(:)])+self.lambda(2)*norm([para_bs.kernel.para.h(:)]);

            % penalty=self.lambda*norm([para_alpha.kernel.kernel.para.h(:)]);
        case {'L1', 'l1'}

            penalty = self.lambda(1) * norm([para_xi.kernel.para.h(:); para_alpha.kernel.para.h(:)], 1) + self.lambda(2) * norm([para_bs.kernel.para.h(:)], 1);
            % elseif strcmpi(self.para_xi.type,'tstudent+peak')
            %     penalty=self.lambda(1)*norm([para_xi.kernel.para.h(:);para_xi.kernel.para.h(:);para_alpha.kernel.para.h(:)],1)+self.lambda(2)*norm([para_bs.kernel.para.h(:)],1);

        case {'L2,L1', 'l2,l1'}

            penalty = self.lambda(1) * norm([para_xi.kernel.para.h(:); para_alpha.kernel.para.h(:)], 2) + self.lambda(2) * norm([para_bs.kernel.para.h(:)], 1);
            % elseif strcmpi(self.para_xi.type,'tstudent+peak')
            %     penalty=self.lambda(1)*norm([para_xi.kernel.para.h(:);para_xi.kernel.para.h(:);para_alpha.kernel.para.h(:)],2)+self.lambda(2)*norm([para_bs.kernel.para.h(:)],1);
