...
if isempty(x0)
    ...
    switch self.para_fit.optfun
        ...
        case 'lsqnonlin'
            ...
            % 'ScaleProblem', self.para_fit.ScaleProblem, ...
            % options = optimoptions('lsqnonlin', 'MaxIterations', self.para_fit.MaxIterations, ...
            %     'MaxFunctionEvaluations', self.para_fit.MaxFunctionEvaluations, 'Display', self.para_fit.Display, ...
            %     'Algorithm', self.para_fit.Algorithm, ...
            %     'UseParallel', self.para_fit.UseParallel, ...
            %     'ScaleProblem', self.para_fit.ScaleProblem);
            % options = optimoptions('lsqnonlin', 'Display', self.para_fit.Display,...
            %     'Algorithm', self.para_fit.Algorithm,'UseParallel', self.para_fit.UseParallel);
            % self.para_fit.MaxIterations=options.MaxIterations;
            % options = optimoptions('lsqnonlin','Algorithm', self.para_fit.Algorithm);
            % options = struct_copy_fields(options,self.para_fit,false);
            self.model.loss_history = nan(self.para_fit.MaxIterations + 1000, 1);

            if ~isempty(nonlcon)
                % [x, loss] = lsqnonlin(@(x) lostfun(x), ...
            ...
            %     [x, loss] =  patternsearch(fun,x0,A,b,Aeq,beq,lb,ub)
        otherwise
    ...
    % idx_jocobian_nonzero = ~(sum(abs(jacobian), 1) < 1e3 .* eps);
    % ci_compat = NaN(size(x, 1), 2);
    % se = NaN(size(x, 1), 1);
    % [ci_compat(idx_jocobian_nonzero, :), se(idx_jocobian_nonzero, 1)] = nlpar_ci_se(x(idx_jocobian_nonzero, :), residual, 'jacobian', jacobian(:, idx_jocobian_nonzero));
